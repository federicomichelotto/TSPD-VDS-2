...
void parse_truck_travel_data(instance *inst)
{
    ...
    while (fgets(line, sizeof(line), fp) != NULL)
    {
        ...
        // inst->truck_times[i][j] = strtod(strtok(NULL, delimiters), NULL) + TRUCK_DELIVERY_TIME; // travel time
...
void initialize_instance(instance *inst)
{
    ...
    srand(timestamp.tv_nsec); // Initialize a random seed for the random numbers generator
    inst->model_type = 0;
    ...
    for (int i = 0; i < inst->dimension - 1; i++)
    {
        for (int j = 1; j < inst->dimension - 1; j++)
        {
            ...
            for (int k = 1; k < inst->dimension; k++)
            {
                ...
                if (inst->param.MHD == 1) {
                    if (inst->min_feas_time_drone[i][j][k] > POIKONEN_UAV_ENDURANCE) {
...
int count_all_feasible_truck_legs_no_recursion(instance *inst) {
    ...
    for (int i = 0; i < inst->dimension - 1; i++) {
        for (int k = 1; k < inst->dimension; k++) {
            ...
            const int count_partial = count_truck_legs_no_recursion_save2(inst, i, k, &(inst->feasible_truck_legs), &(inst->feasible_truck_legs_times), inst->number_feasible_truck_legs);
            // count += count_partial;
...
int count_truck_legs_no_recursion_save(instance *inst, const int i, const int k, int*** feasible_truck_legs, double** feasible_truck_legs_times, const int init_truck_legs_count) {
    ...
    for (int j = 1; j < inst->dimension - 1; j++) {
        ...
        int *truck_legs_idx_with_prev_length = NULL;
        int *truck_legs_idx_with_curr_length = (int*)malloc(1 * sizeof(int));;
        ...
        int truck_leg_length = 2;
        {
            ...
            for (int old_id = begin_truck_legs_ik; old_id < truck_legs_count_overall; old_id++) {
                if ((*feasible_truck_legs)[old_id][0] != truck_leg_length)
                    continue;
                int compare_value = memcmp(current_truck_leg + 1, (*feasible_truck_legs)[old_id] + 1, truck_leg_length * sizeof(int));
                // int compare_value = 0;
                // for (int v = 1; v <= truck_leg_length; v++) {
                //     if ((*feasible_truck_legs)[old_id][v] != current_truck_leg[v]) {
                //         compare_value = 1;
                //         break;
                //     }
                // }
            ...
            if (!found) {
                ...
                *feasible_truck_legs_times = realloc(*feasible_truck_legs_times, (truck_legs_count_overall + 1) * sizeof(double));
                ...
                truck_legs_idx_with_curr_length[0] = truck_legs_count_overall;
                truck_legs_count_ik++;
                truck_legs_count_overall++;
            }
            else {
                truck_legs_idx_with_curr_length[0] = old_id_copy;
            }
            count_truck_legs_with_length[truck_leg_length]++;
            printf("truck_leg: ");
            for (int tmp_cust = 1; tmp_cust < truck_leg_length + 1; tmp_cust++) {
                printf("%d ", current_truck_leg[tmp_cust]);
            }
            printf(" ID = %d\n", truck_legs_idx_with_curr_length[0]);
        }

        truck_leg_length++;

        for (; truck_leg_length <= inst->dimension - 1; truck_leg_length++) {
            free(truck_legs_idx_with_prev_length);
            truck_legs_idx_with_prev_length = truck_legs_idx_with_curr_length;
            truck_legs_idx_with_curr_length = NULL;

            printf("ID LIST lenght %d = ", truck_leg_length - 1);
            for (int id = 0; id < count_truck_legs_with_length[truck_leg_length - 1]; id++) {
                printf("%d ", truck_legs_idx_with_prev_length[id]);
            }
            printf("\n");
            char c;
            scanf("%c", &c);
            ...
            for (int id_prev_l = 0; id_prev_l < count_truck_legs_with_length[truck_leg_length - 1]; id_prev_l++) {
                int prev_leg_id = truck_legs_idx_with_prev_length[id_prev_l];
                ...
                printf("Try to extend truck leg with ID %d\n", prev_leg_id);
                ...
                for (int cust = 1; cust < inst->dimension - 1; cust++) {
                    ...
                    // double true2_truck_leg_time = 0.0f;
                    // for (int cust_id = 1; cust_id < truck_leg_length; cust_id++) {
                    //     true2_truck_leg_time += inst->truck_times[current_truck_leg[cust_id]][current_truck_leg[cust_id + 1]];
                    // }
                    // assert(fabs(true2_truck_leg_time - new_truck_leg_time) < 1e-4);

                    truck_legs_idx_with_curr_length = (int*)realloc(truck_legs_idx_with_curr_length, (count_truck_legs_with_length[truck_leg_length] + 1) * sizeof(int));
                    ...
                    if (!found) {
                        ...
                        *feasible_truck_legs_times = realloc(*feasible_truck_legs_times, (truck_legs_count_overall + 1) * sizeof(double));
                        ...
                        truck_legs_idx_with_curr_length[count_truck_legs_with_length[truck_leg_length]] = truck_legs_count_overall;
                        truck_legs_count_ik++;
                        truck_legs_count_overall++;
                    }
                    else {
                        truck_legs_idx_with_curr_length[count_truck_legs_with_length[truck_leg_length]] = old_id_copy;
                    }
                    count_truck_legs_with_length[truck_leg_length]++;
                    printf("truck_leg: ");
                    for (int tmp_cust = 1; tmp_cust < truck_leg_length + 1; tmp_cust++) {
                    ...
                    printf(" ID = %d\n", truck_legs_idx_with_curr_length[count_truck_legs_with_length[truck_leg_length] - 1]);
                }
        ...
        free(truck_legs_idx_with_prev_length);
        free(truck_legs_idx_with_curr_length);
    }
...
int count_truck_legs_no_recursion_save2(instance *inst, const int i, const int k, int*** feasible_truck_legs, double** feasible_truck_legs_times, const int init_truck_legs_count) {
    // char c;
    // scanf("%c", &c);
    if (i == k) return 0;
    if (i == inst->dimension - 1 || k == 0) return 0;
    ...
    for (int j = 1; j < inst->dimension - 1; j++) {
        ...
        int end_prev_length = truck_legs_count_overall;
        int begin_prev_length = end_prev_length - count_truck_legs_with_length[truck_leg_length - 1];
        ...
        for (; truck_leg_length <= inst->dimension - 1; truck_leg_length++) {
            ...
            for (int prev_leg_id = begin_prev_length; prev_leg_id < end_prev_length; prev_leg_id++) {
                assert((*feasible_truck_legs)[prev_leg_id][0] == truck_leg_length - 1);
                assert((*feasible_truck_legs)[prev_leg_id][1] == i);
                assert((*feasible_truck_legs)[prev_leg_id][truck_leg_length - 1] == k);
            ...
            end_prev_length = truck_legs_count_overall;
            begin_prev_length = truck_legs_count_overall - count_truck_legs_with_length[truck_leg_length];
