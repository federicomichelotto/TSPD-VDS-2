...
int tsp_solver(instance_TSP *inst)
{
    ...
    if (error)
        print_error("Error in CPXopenCPLEX() (TSP)\n");
    ...
    for (int i = 0; i < inst->dimension; i++)
    {
        for (int j = 0; j < inst->dimension; j++)
        {
            ...
            if (i < inst->dimension-1)
                obj += TRUCK_DELIVERY_TIME;
            double lb = 0.0;
            double ub = 1.0;
            if (i == j)
                ub = 0.0;
            if (inst->weights[i][j] == DBL_MAX)
                ub = 0.0;
