...
double euclDist(double xx1, double xx2, double yy1, double yy2) {
	double quad = (xx1 - xx2) * (xx1 - xx2) + (yy1 - yy2) * (yy1 - yy2);
	return pow(quad, 0.5);
}

double dist_MAN_2D(int i, int j, instance *inst)
{
	double dx = fabs(inst->nodes[i].x - inst->nodes[j].x);
	double dy = fabs(inst->nodes[i].y - inst->nodes[j].y);
	return dx + dy;
}
...
// retrieve the position of the decision var. alpha(i) in the cplex vector
int alpha_pos(int i, instance *inst)
{
	if (i < 0 || i >= inst->dimension)
		print_error("Error in alpha_pos: i must be in the range [0, inst->dimension]");
	return z_pos(inst->dimension - 1, inst->dimension - 1, inst->dimension - 1, inst) + 1 + i;
}

// retrieve the position of the decision var. beta(i) in the cplex vector
int beta_pos(int i, instance *inst)
{
	if (i < 0 || i >= inst->dimension)
		print_error("Error in beta_pos: i must be in the range [0, inst->dimension]");
	return alpha_pos(inst->dimension - 1, inst) + 1 + i;
}
...
void gather_solution(instance *inst, const double *xstar)
{
	...
	while (node != inst->dimension - 1)
	{
		...
		{
			if (xstar[xTruck_pos(node, j, inst)] > 0.5)
			{
				flag = 1;
				inst->truck_seq[n++] = j;
				node = j;
		...
		if (!flag || n > inst->dimension)
			print_error("Error in gather_solution()\n");
	}
	...
	while (node != inst->dimension - 1)
	{
		...
		for (int j = 1; j < inst->dimension; j++)
		{
			if (xstar[xDrone_pos(node, j, inst)] > 0.5)
			{
				flag = 1;
				inst->drone_seq[n++] = j;
				node = j;
...
void build_model(CPXENVptr env, CPXLPptr lp, instance *inst, double bigM)
{
    ...
    for (int i = 0; i < inst->dimension; i++)
    {
    	...
    	double obj = UAV_LAUNCH_TIME + UAV_RETRIEVAL_TIME;
    	double lb = 0.0;
    	double ub = 1.0;
    	if (i == 0 || i == inst->dimension - 1)
    		ub = 0.0;
    	if (inst->nodes[i].weight > UAV_WEIGHT_LIMIT)
    		ub = 0.0;
    	if (CPXnewcols(env, lp, 1, &obj, &lb, &ub, &binary, cname))
    ...
    // Add alpha variables for each customer i in N
    // alpha(i) = 1 if the truck delivery occurs right after the truck arrival at node i (before synchronization)
    for (int i = 0; i < inst->dimension; i++)
    {
    	sprintf(cname[0], "alpha(%d)", i);
    	double obj = 0.0;
    	double lb = 0.0;
    	double ub = 1.0;
    	if (i == 0)
    		lb = 1.0;
    	if (CPXnewcols(env, lp, 1, &obj, &lb, &ub, &binary, cname))
    		print_error("wrong CPXnewcols on alpha variables");
    	if (CPXgetnumcols(env, lp) - 1 != alpha_pos(i, inst))
    		print_error("wrong position for alpha variables");
    }

    // Add beta variables for each customer i in N
    // beta(i) = 1 if the truck delivery occurs after the truck and drone synchronization at node i
    for (int i = 0; i < inst->dimension; i++)
    {
    	sprintf(cname[0], "beta(%d)", i);
    	double obj = 0.0;
    	double lb = 0.0;
    	double ub = 1.0;
    	if (i == inst->dimension - 1)
    		lb = 1.0;
    	if (CPXnewcols(env, lp, 1, &obj, &lb, &ub, &binary, cname))
    		print_error("wrong CPXnewcols on beta variables");
    	if (CPXgetnumcols(env, lp) - 1 != beta_pos(i, inst))
    		print_error("wrong position for beta variables");
    }
    ...
    // Each combined customer must be served before (alpha = 1) or after (beta = 1) sync
    for (int i = 0; i < inst->dimension; i++){
    	int row = CPXgetnumrows(env, lp); // get the number of rows inside the model
    	double rhs = 0.0;
        char sense = 'E'; // E stands for equality constraint
        sprintf(rname[0], "yC(%d)", i);
        if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, rname))
        	print_error("wrong CPXnewrows [yC]");
        if (CPXchgcoef(env, lp, row, yC_pos(i, inst), 1.0))
        	print_error("wrong CPXchgcoef [yC]");
        if (CPXchgcoef(env, lp, row, alpha_pos(i, inst), -1.0))
        	print_error("wrong CPXchgcoef [yC]");
        if (CPXchgcoef(env, lp, row, beta_pos(i, inst), -1.0))
        	print_error("wrong CPXchgcoef [yC]");
    }
    ...
    for (int i = 0; i < inst->dimension; i++)
    {
    	for (int j = 0; j < inst->dimension; j++)
    	{
            ...
            if (CPXchgcoef(env, lp, row, xTruck_pos(i, j, inst), -M))
            	print_error("wrong CPXchgcoef [truck_time_constraint]");
            // new part
            if (CPXchgcoef(env, lp, row, yT_pos(i, inst), -TRUCK_DELIVERY_TIME))
            	print_error("wrong CPXchgcoef [truck_time_constraint]");
            if (CPXchgcoef(env, lp, row, beta_pos(i, inst), -TRUCK_DELIVERY_TIME))
            	print_error("wrong CPXchgcoef [truck_time_constraint]");
            if (CPXchgcoef(env, lp, row, alpha_pos(j, inst), -TRUCK_DELIVERY_TIME))
            	print_error("wrong CPXchgcoef [truck_time_constraint]");
        }
    }
    ...
    //break symmetry alpha beta
    for (int i = 0; i < inst->dimension-1; i++){
    	for (int j = 1; j < inst->dimension-1; j++){
    		int row = CPXgetnumrows(env, lp);
    		double rhs = 2.0;
	        char sense = 'L'; // "<="
	        sprintf(rname[0], "symmmetry_alpha_beta_ij(%d,%d)",i,j);
	        if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, rname))
	        	print_error("wrong CPXnewrows [symmmetry_alpha_beta_ij]");
	        if (CPXchgcoef(env, lp, row, alpha_pos(j, inst), 1.0))
        		print_error("wrong CPXchgcoef [symmmetry_alpha_beta_ij]");
	        if (CPXchgcoef(env, lp, row, xTruck_pos(i,j, inst), -1.0))
        		print_error("wrong CPXchgcoef [symmmetry_alpha_beta_ij]");
        	if (CPXchgcoef(env, lp, row, xDrone_pos(i,j, inst), -1.0))
        		print_error("wrong CPXchgcoef [symmmetry_alpha_beta_ij]");
...
void retrieve_max_i_drone_leg_times(instance *inst)
{
	...
	for (int i = 0; i < inst->dimension; i++)
	{
		...
		for (int j = 1; j < inst->dimension - 1; j++)
		{
			if (inst->nodes[j].weight > UAV_WEIGHT_LIMIT)
				continue;
			for (int k = 1; k < inst->dimension; k++)
			{
